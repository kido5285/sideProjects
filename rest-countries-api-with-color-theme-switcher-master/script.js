...
function search(value) {
    setTimeout(() => {
        ...
        if(background === 'rgb(250, 250, 250)') {
            ...
            document.querySelector('.no-result').style.color = '#000';
        } else {
            Array.from(document.querySelectorAll('.country')).forEach(elem => elem.classList.add('dark'));
            document.querySelector('.no-result').style.color = '#fff';
        }
    }, 2000)
    var theFullData = '';
    if(value.trim() === '') {
        ...
        .then(data => {
            if(data.status == 404) {
                theFullData = '<span class="no-result">No result:(</span>'
...
function theme(obj) {
    ...
    if(background === 'rgb(250, 250, 250)') {
        ...
        document.querySelector('.real-page2').style.minHeight = '100vh';       
